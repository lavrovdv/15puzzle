...
function A(puzzle){
    ...
    var close_list = [];
    var current = null;
    var terminate = get_terminate_state(puzzle);

    while(!is_terminate(open_list, terminate) && open_list.length > 0){
    ...
    function is_terminate(openlist, terminate){
        for(var i = 0; i < openlist.length; i++){
            if (is_equal(openlist[i].puzzle, terminate)){return true}
        }
        return false;
    }
    ...
    function remove_state(openlist, element){
        for (var i = 0; i < openlist.length; i++) {
            if(openlist[i] == element){openlist.slice(i,1)}
        }
    }
}
